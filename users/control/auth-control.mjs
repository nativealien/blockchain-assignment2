...
export const register = asyncHandler( async (req, res, next) => {
    try {
        if(!req.body.name || !req.body.email || !req.body.password || !req.body.password){
            res.status(201).json(Response.post('Credentials missing...', null))  
        }else{
        ...
        res.status(201).json(Response.post('Something went wrong...', null))
    }
})

export const login = asyncHandler( async (req, res, next) => {
    const { email, password } = req.body
    if(!email || !password) res.status(201).json(Response.post('Wrong Email or Password...', null))

    const user = await User.findOne({ email }).select('+password')
    if(!user) res.status(201).json(Response.post('Wrong Email or Password...', null))

    const check = await user.validatePassword(password)
    if(!check) res.status(201).json(Response.post('Wrong Email or Password...', null))
